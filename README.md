...
Skorlama modeli, 3 katmanlı **MLP (Multi-Layer Perceptron)** — yani tam bağlantılı ileri beslemeli sinir ağıdır. Her protein–peptid çifti için üretilen dense özellik vektörünü girdi olarak alır ve 0–1 arası bir etkileşim skoru üretir. Aktivasyon fonksiyonu olarak SiLU kullanılır.

> **Not:** Eğitim Apple Silicon (M-serisi) çipler üzerinde [Apple MLX](https://github.com/ml-explore/mlx) framework'ü ile gerçekleştirilmiştir. MLX, PyTorch benzeri bir API sunan Apple'ın makine öğrenmesi kütüphanesidir; model mimarisinin kendisi değil, eğitim altyapısıdır.

| Parametre | Değer |
|-----------|-------|
| Mimari | MLP (3 katman, SiLU aktivasyon) |
| Gizli boyut | 192 |
| Loss | BCE (0.5) + Pairwise Ranking Loss (margin=0.2) |
| Öğrenme hızı | 0.001 |
| Weight decay | 1e-4 |
| Batch boyutu | 512 |
| Erken durdurma | Val MRR ile, patience=12 |
| Eşik seçimi | MCC-optimal threshold (0.59) |
| Seed | 42 |
...
Özellik ihracı (yapı + sekans + arayüz + yerel yoğunluk → dense vektör)
    │
    ▼
MLP eğitimi (BCE + pairwise ranking loss; val MRR ile erken durdurma)
    │
    ▼
...
5. **MLX** — Apple Machine Learning Research. [ml-explore/mlx](https://github.com/ml-explore/mlx)
