...
Examples:
  ...
  peptidquantum run --pdb 1ABC --no-pymol
    ...
    # Processing options
    run_parser.add_argument('--pocket-radius', type=float, default=8.0,
                           help='Pocket extraction radius in Angstroms (default: 8.0)')
    
    # Interaction extraction
    run_parser.add_argument('--no-arpeggio', action='store_true',
                           help='Disable Arpeggio interaction extraction')
    run_parser.add_argument('--no-plip', action='store_true',
                           help='Disable PLIP interaction extraction')
    
    # Visualization
    run_parser.add_argument('--no-pymol', action='store_true',
                           help='Disable PyMOL figure generation')
    run_parser.add_argument('--no-report', action='store_true',
                           help='Disable HTML report generation')
    run_parser.add_argument('--no-viewer', action='store_true',
                           help='Disable standalone 3Dmol.js viewer')
    
    # Output
    run_parser.add_argument('--output', type=Path, default='outputs',
                           help='Output directory (default: outputs)')
    run_parser.add_argument('--cache', type=Path, default='data/cache',
    ...
    config_parser.add_argument('--output', type=Path, default='config.json',
                              help='Output config file (default: config.json)')
